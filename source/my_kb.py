...
def create_kb():
    ...
    prolog.assertz("num_of_dead(C, N) :- findall(V, (victim(V, C), victimization(V, homicide), L), count(L, N)")
    prolog.assertz("is_homicide(C) :- victim(C, V), victimization(V, homicide).")
    prolog.assertz("is_killed(V) :- victim(C, V), is_homicide(C).")
    prolog.asserta("num_of_arrest(C, N) :- findall(S, has_arrest(C, S), L), length(L, N)")
